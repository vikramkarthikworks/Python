...
# Self-Adaptive Jaya Algorithm execution
for iteration in range(num_iterations):
    ...
    adapt_factor = np.exp(-iteration / num_iterations)  # Decreases over time

    for i in range(population_size):
        r1, r2 = np.random.rand(), np.random.rand()
        new_solution = population[i] + adapt_factor * (r1 * (best_solution - abs(population[i])) - r2 * (abs(worst_solution - population[i])))
...
plt.title("Optimized Edge Node Positions in IoT Network (Self-Adaptive Jaya Algorithm)")
plt.legend()
plt.grid(True)
plt.show()
